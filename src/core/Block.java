...
/** Block stores an array of bytes from a block in a BlockFile, and
    provides methods to access and update values represented in the
    array of bytes.
*/
...
public class  Block{
    ...
    public void setByte(byte value, int index){ bytes[index] = value; }
	...
	for (int i=0; i<length; i++){
	    if (bytes[offset1+i]+128 < other.bytes[offset2+i]+128) return -1;
	    if (bytes[offset1+i]+128 > other.bytes[offset2+i]+128) return 1;
	}
	return 0;
    }
	...
	for (int i=0; i<value.length; i++){
	    if (value[i]+128 < bytes[offset+i]+128) return -1;
	    if (value[i]+128 > bytes[offset+i]+128) return 1;
	}
	return 0;
    }
